...
@section('content')
    ...
    <form method="POST" action="{{ route('estacions.store') }}">
        @csrf

        <label for="EstNombre">Nombre de la estación:</label>
        <input type="text" name="EstNombre" value="{{ old('EstNombre') }}" required>
        @error('EstNombre') <div class="error">{{ $message }}</div> @enderror

        <label for="EstUbicacion">Ubicación:</label>
        <input type="text" name="EstUbicacion" value="{{ old('EstUbicacion') }}" required>
